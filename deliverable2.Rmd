...
![](C:/Upenn/Practicum/markdown_img/Passenger-bus-APC-1024x576.jpg)
...
![](C:/Upenn/Practicum/markdown_img/ds1.jpg)
...
![](C:/Upenn/Practicum/markdown_img/ds2.jpg)
...
![](C:/Upenn/Practicum/markdown_img/AnnualTrend.png)

## 2.2 Ridership Pattern in Subdivisions

After knowing the trend of city-wide ridership change, the next question is how the ridership changed across the city: which area experienced ridership increase and which experienced decrease. To understand whether the changes are significant, we first looked at the existing riderhsip patterns according to the general typology and then use neighborhoods in Austin to see the ridership spatial distribution. 
...
![](C:/Upenn/Practicum/markdown_img/typo.png){width=400px}![](C:/Upenn/Practicum/markdown_img/typo_map.png){width=400px}
...
![](C:/Upenn/Practicum/markdown_img/Ridership.JPG){width=500px}
...
![](C:/Upenn/Practicum/markdown_img/ut_change.png)
...
![](C:/Upenn/Practicum/markdown_img/govalle_change.png)
...
![](C:/Upenn/Practicum/markdown_img/zilker_change.png)
...
![](C:/Upenn/Practicum/markdown_img/routetypePNG.png)
...
![](C:/Upenn/Practicum/markdown_img/hotlines_ba.png)
...
![](C:/Upenn/Practicum/markdown_img/line20.png)

Below is the analysis for Line 801.
![](C:/Upenn/Practicum/markdown_img/line801.png)
...
![](C:/Upenn/Practicum/markdown_img/Amenities.JPG){width=500px}![](C:/Upenn/Practicum/markdown_img/Ridership.JPG){width=300px}
...
![](C:/Upenn/Practicum/markdown_img/Commercial.JPG){width=250px}![](C:/Upenn/Practicum/markdown_img/Civic.JPG){width=250px}![](C:/Upenn/Practicum/markdown_img/Ridership.JPG){width=300px}

### 2.4.3 Demographics
Lastly, demographics is the ultimate features that we take into account.In this case, some basic characteristics about the areas might influence the ridership such as race, income and age. Here we will use vehicle ownership, more specifically, households without vehicle, as an exploratory feature.
Since demographic data has the unit of census tract, it reveals more details than the ridership map. Mostly, in the neighborhoods with high ridership, there are some census tract where more households have no vehicle ownership. In this aspect, it clearly indicates the importance of our application which allows planners to know the ridership change according to demographic features, which can effectively improve resource allocation considering social equity.
![](C:/Upenn/Practicum/markdown_img/Noveh.JPG){width=300px}![](C:/Upenn/Practicum/markdown_img/Ridership.JPG){width=300px}
...
![](C:/Upenn/Practicum/markdown_img/Radius.png){width=300px}
...
For route information, the percentage of each route type passing each bus stop is calculated. The number of shifts and number of buses going through each stop on a given week is also calculated. We also included the route fixed effect, i.e. each route number is marked as a new feature for all the stops (For example, for the "route10" column, all stops on route 10 will be marked as 1, all other stops are marked as 0).
...
![](C:/Upenn/Practicum/markdown_img/features.png)
...
![](C:/Upenn/Practicum/markdown_img/positive.png)

![](C:/Upenn/Practicum/markdown_img/negative.png)
...
![](C:/Upenn/Practicum/markdown_img/cor_matrix.png)
...
![](C:/Upenn/Practicum/markdown_img/MAPE_overall_nhood.JPG){width=400px}![](C:/Upenn/Practicum/markdown_img/MAE_overall_nhood.JPG){width=400px}
...
![](C:/Upenn/Practicum/markdown_img/PreVSObs_nhood.JPG){width=350px}
...
![](C:/Upenn/Practicum/markdown_img/Map_nhood_MAPE_lm.JPG){width=400px}![](C:/Upenn/Practicum/markdown_img/Map_nhood_MAPE_glm.JPG){width=400px}
![](C:/Upenn/Practicum/markdown_img/Map_nhood_MAPE_rf.JPG){width=400px}![](C:/Upenn/Practicum/markdown_img/Map_nhood_MAPE_xgb.JPG){width=400px}
...
![](C:/Upenn/Practicum/markdown_img/21_half.JPG){width=600px}
...
![](C:/Upenn/Practicum/markdown_img/18_quarter.JPG){width=600px}
...
![](C:/Upenn/Practicum/markdown_img/19_eight.JPG){width=600px}
...
![](C:/Upenn/Practicum/markdown_img/22_half.JPG){width=500px}
![](C:/Upenn/Practicum/markdown_img/20_quarter.JPG){width=500px}
![](C:/Upenn/Practicum/markdown_img/23_eighth.JPG){width=500px}
...
![](C:/Upenn/Practicum/markdown_img/Poverty.JPG){width=500px}
...
![](C:/Upenn/Practicum/markdown_img/Poverty_va.png)

# 4. Application

## 4.1 Why scenario planning?
As we mentioned in the beginning, the current workflow of redesigning a bus system lacks efficiency. Most of the analytical work and modeling on ridership was outsourced and for in-house planners, it is hard to test out different planning scenarios on the fly. This inspired us to create an application that helps planners to have a better understanding of the ridership change through different scenarios.
In addition, scenario planning provides planners a tool to think holistically into the future. It gives planners the opportunity to investigate a number of future possibilities without being bogged down by how likely these scenarios might actually happen. Such a procedure offers planners the system thinking that reduces future risks and capture potential opportunities.
Our model discussed above already can predict ridership based on given inputs including route frequency, building area, land use, etc. In order to see if our model can achieve our goal of scenario prediction, we have tested the model on three different planning scenarios and presented the predicted ridership change in ridership below.

## 4.2 Three different scenarios for ridership change
(TO BE UPDATED WITH TECH BOOM IN AUSTIN)
![](Feeder route and stops img)
### 4.2.1 Scenario 1 - Bus Frequency and Ridership
In this scenario, we investigated the relationship between change of bus frequency and ridership.  The frequency of all Feeder routes was increased to make them becomes high frequency routes (meaning with a headway that is less than 15 minutes). The overall ridership has increased after the increase of frequency. However, it is less expected that the majority of the stops would experience ridership increase while a few stops in the east side of the city demonstrate decrease in ridership.
![](C:/Upenn/Practicum/markdown_img/new_scenario1.png)

### 4.2.2 Scenario 2 - Building Areas and Ridership
...
Similar to the increased frequency scenario, the change of the ridership doesn’t seem to follow a pattern with the consistent change of building area. It is worth further investigation that most terminal stops experience ridership increase.
![](C:/Upenn/Practicum/markdown_img/new_scenarios2.png)

### 4.2.3 Scenario 3 - Land use and Ridership
...
![](C:/Upenn/Practicum/markdown_img/new_scenario3.png)
...
![](C:/Upenn/Practicum/markdown_img/web1.png)

![](C:/Upenn/Practicum/markdown_img/web2.png)

![](C:/Upenn/Practicum/markdown_img/web3.1.png)

![](C:/Upenn/Practicum/markdown_img/web3.png)
...
![](C:/Upenn/Practicum/markdown_img/web4.png)
...
typo_bar <- ggplot(data = all_x4_normalize %>% 
  ...
  geom_text(aes(label=round(mean_boarding,1)), vjust=1.6, color="white", size=5)+
...
```{r}
typo_map = ggplot() +
  geom_sf(data =st_union(nhood), fill = "#bdd7e7", color="transparent") +
  geom_sf(data = CBD_bound, fill = "#2171b5" , color="transparent") +
  geom_sf(data = UTAustin_bound, fill = "#6baed6", color="transparent")+
  mapTheme()
```
...
print(typo_bar, vp=viewport(0.34,0.5, 0.68,1))
print(typo_map, vp=viewport(0.83,0.7,0.57,0.57))
```
...
```{r}
# bus frequency
bus_count = disagn2 %>% group_by(STOP_ID) %>% summarise(count = n()/5)
```
...
route_fix[is.na(route_fix)] <- 0
colnames(route_fix) <- paste("r", colnames(route_fix), sep = "")
route_fix <- route_fix %>% as.data.frame()
```
...
summary(all_x4_normalize$morning_on)
summary(all_x4_normalize$morning_off)
summary(all_x4_normalize$morning_load)

summary(all_x4_normalize$afternoon_on)
summary(all_x4_normalize$afternnon_off)
summary(all_x4_normalize$afternoon_load)

summary(all_x4_normalize$eve_on)
summary(all_x4_normalize$eve_off)
summary(all_x4_normalize$eve_load)
```
...
```{r}
route_names <- colnames(subset(route_fix, select = -c(r)))
