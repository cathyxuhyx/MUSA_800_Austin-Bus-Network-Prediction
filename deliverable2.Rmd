...
```{r}
lmreg.step <- lm(mean_on ~ .,data = all_x4_normalize %>% st_drop_geometry() %>% drop_na() %>% select(-STOP_ID))

step.model <- lmreg.step %>% stepAIC(trace=FALSE)
summary(step.model)
#coef(step.model)
```
...
lmreg <- lm(mean_on ~ .,data = all_x4_normalize%>% st_drop_geometry() %>% select(-STOP_ID))
summary(lmreg)
lm_model0 <-
  all_x4_normalize %>%
  mutate(ridership.Predict = predict(lmreg, all_x4_normalize)) %>%
  select(ridership.Predict) %>%
  st_drop_geometry()
```
...
```{r}
#plot the change in ridership
ggplot()+
  geom_sf(data = nhood, color = 'grey80',fill = 'grey80') +
  geom_sf(data = st_centroid(all5), aes(color = building_area)) +
  labs(title = "buildling area 0") +
  mapTheme()+theme(legend.position="bottom")
```
