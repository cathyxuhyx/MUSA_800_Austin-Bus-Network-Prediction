...
>>>>>>> b0b1fbd2e8a966865db12fa36b7993d915d73460
```
...
First, let us look at the Kmeans analysis before the CapRemap. We group the disaggregated data 
```{r}
disagn1_km = disagn1 %>%
  group_by(ROUTE) %>%
...
k means find the clusters, number determined through 26 criteria test, see appendix.
```{r}
...
k means find clusters, number determined through 26 criteria test, see appendix.
```{r}
...
join data to the original dataset
```{r}
...
select route patterns id that has the majority instances
